...
import my.app.playm.model.decode.XugglerExporter;
...
public class VideoServiceImpl implements VideoService{
    ...
    Player player;
    @Autowired
    PlayerOnlyVideo onlyVideoPlayer;
    @Autowired
    PlayerAudioVideo audioVideoPlayer;

    public void decodeVideo(String source) {
        Dispatcher.onStartNewVideo(source);
        new Thread(() -> {
            Data.decoder.decodeAndEvent(source);
    ...
    public void serializeImageFrames() {
        ...
        try (FileOutputStream file = new FileOutputStream("video.data");
            ...
            out.writeObject(Data.videoRepo.getList());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void export(String target){
        new XugglerExporter().export(target);
    }
    public void decodeAudio(String source){
        Data.decoderAudio.decodeAudio(source);
        if(!Data.decoderAudio.isAudio()){
