...
public class TimerThread implements Timer {
    ...
    private Runnable playForward = () -> {
        ...
        if (Data.range.isEnd()) {
            if (Data.currentFrame > Data.range.getEnd()) {
                log.debug(String.format("Frame %d is more than range end - %d", Data.currentFrame, Data.range.getEnd()));
                reset();
            }
        } else if (Data.playMode == PlaybackMode.ACTUAL) {
            if (Data.currentFrame > Data.videoRepo.size() - 1) reset();
            } else {
            if (Data.currentFrame > Data.range.getOrgEnd()) reset();
            }
    ...
    private Runnable playBackward = () -> {
        // return back to start frame if reaches the end frame (looping playback)
        if (--Data.currentFrame < 0) Data.currentFrame = Data.videoRepo.size() - 1;
        if (Data.currentFrame % Data.frameStep != 0) return;
    ...
    public void reset() {
        ...
        if (Data.decoderAudio.isAudio()) audioPlayer.seek(audioPlayer.getStartTime());
